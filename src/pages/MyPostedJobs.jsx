...
const modernDelete = (id) => {
  toast((t) => (
   ...
   <p>   Are you <b>Sure</b>?</p>
      ...
      <button className='btn btn-outline btn-error' onClick={() => toast.dismiss(t.id)}>Cancel</button>
      </div>
    </div>
  ));
};
                   ...
                   {format(new Date(job.deadline),'P')
                    ...
                    <td className='px-4 py-4 text-sm whitespace-nowrap'>
                      <div className='flex items-center gap-x-2'>
                        <p
                          className={`px-3 py-1  ${job.category==="Digital Marketing" &&'bg-blue-400'} ${job.category==="Graphics Design" &&'bg-purple-400'} ${job.category==="Web Development" &&'bg-red-200'} bg-blue-100/60 text-xs  rounded-full`}
                        >
                         {job.category}
                        </p>
                      </div>
                    ...
                    <td className='px-4 py-4 text-sm text-gray-500  whitespace-nowrap'>
                    {job.description ? `${job.description.substring(0, 70)}...` : "No description available"}
                    </td>
                    <td className='px-4 py-4 text-sm whitespace-nowrap'>
